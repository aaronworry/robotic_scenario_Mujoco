...
class Arm_FK():
    ...
    def compute_geometric_jacobian_derivative(self, q, dq, T_ee):
        ...
        for i in range(self.nq):
            ...
            for j in range(i + 1):
                p_prev = p_list[j]
                dz_j = np.cross(w_list[j], z_list[j])
                dr_j = v_list[-1] - v_list[j]
                ...
                if i == self.nq - 1:
                    ...
                    J_ee[3:, j] = z_list[-1]
                    dJ_ee[:3, j] = np.cross(dz_j, p_ee - p_prev) + np.cross(z_list[-1], v_ee - v_list[j])
                    dJ_ee[3:, j] = dz_j
...
if __name__ == "__main__":
    ...
    print(fk.compute_jacobian([np.pi/6, np.pi/6, np.pi/6], np.eye(4)))
