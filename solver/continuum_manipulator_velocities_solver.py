...
class VelocityOptimizer():
    ...
    def create_opt(self):
        ...
        # delta l = J q_dot dt
        for i in range(self.n_cables):
            self.model.addConstr(gp.quicksum(J[i, j] * self.q_dot[i, j] for j in range(self.n_modules)) == 0., name = "cable_delta_length_"+str(i))

        # 最小作用量
        obj = gp.quicksum(self.c[i, j] for i, j in np.ndindex(self.delta_theta.shape))
        self.model.setObjective(obj, GRB.MINIMIZE)
    
    def update_opt(self, J, cable_velocity):
        for i in range(self.n_cables):
            self.model.remove(self.model.getConstrByName("cable_delta_length_"+str(i)))
        self.model.update()
        for i in range(self.n_cables):
            self.model.addConstr(gp.quicksum(J[i, j] * self.q_dot[i, j] for j in range(self.n_modules)) == cable_velocity[i], name = "cable_delta_length_"+str(i))
        self.model.update()
...
if __name__ == "__main__":
    ...
    print(J)
    time_3 = time.time()
    opt.update_opt(J, cable_velocity = np.array([0.03]))
    time_4 = time.time()
    if(opt.solve()):
        print(opt.delta_theta)
        
    print("=================")
    print(time_2- time_1, time_4 - time_3)